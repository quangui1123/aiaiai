...
def _parse_openai_stream_usage(line: str) -> Optional[dict]:
...
class AnthropicAdapter(ProviderAdapter):
    ...
    async def chat_stream(self, req: ChatRequest) -> AsyncIterator[str]:
        ...
        async with self.client.stream(
            ...
            async for line in resp.aiter_lines():
                ...
                if etype == "message_start":
                    ...
                    if text:
                        if not sent_role:
                            yield _openai_sse_chunk(req.model, content="", finish_reason=None)
                            sent_role = True
        ...
        yield _openai_sse_chunk(req.model, finish_reason="stop", usage=usage)
        yield "data: [DONE]\n\n"
